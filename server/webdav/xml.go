...
func writeLockInfo(w io.Writer, token string, ld LockDetails) (int, error) {
	...
	return fmt.Fprintf(w, "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"+
		...
		depth, ld.OwnerXML, timeout, escapeXMLText(token), escapeXMLText(ld.Root),
	)
}
...
// write validates and emits a DAV response as part of a multistatus response element.
...
// writeHeader writes XML declaration and multistatus start element
...
// writeResponse writes a single response element
func (w *multistatusWriter) writeResponse(r *response) error {
	var buf bytes.Buffer
	buf.WriteString("<D:response>")
	for _, href := range r.Href {
	...
	if r.Status != "" {
		...
		xml.EscapeText(&buf, []byte(r.Status))
		buf.WriteString("</D:status>")
	}
	for _, ps := range r.Propstat {
		buf.WriteString("<D:propstat><D:prop>")
		for _, p := range ps.Prop {
			if err := writeProp(&buf, p); err != nil {
				return err
			}
		}
		buf.WriteString("</D:prop><D:status>")
		xml.EscapeText(&buf, []byte(ps.Status))
		buf.WriteString("</D:status>")
		if ps.Error != nil && len(ps.Error.InnerXML) > 0 {
			buf.WriteString("<D:error>")
			buf.Write(ps.Error.InnerXML)
			buf.WriteString("</D:error>")
		}
		if ps.ResponseDescription != "" {
			buf.WriteString("<D:responsedescription>")
			xml.EscapeText(&buf, []byte(ps.ResponseDescription))
			buf.WriteString("</D:responsedescription>")
		}
		buf.WriteString("</D:propstat>")
	}
	if r.Error != nil && len(r.Error.InnerXML) > 0 {
		buf.WriteString("<D:error>")
		buf.Write(r.Error.InnerXML)
		buf.WriteString("</D:error>")
	}
	if r.ResponseDescription != "" {
		buf.WriteString("<D:responsedescription>")
		xml.EscapeText(&buf, []byte(r.ResponseDescription))
		buf.WriteString("</D:responsedescription>")
	}
	buf.WriteString("</D:response>\n")
...
// writeProp writes a property element
func writeProp(buf *bytes.Buffer, p Property) error {
	name := p.XMLName.Local
	if p.XMLName.Space == "DAV:" {
		name = "D:" + name
	} else if p.XMLName.Space != "" {
		// Non-DAV namespace - include xmlns
		buf.WriteString("<")
		buf.WriteString(name)
		buf.WriteString(` xmlns="`)
		xml.EscapeText(buf, []byte(p.XMLName.Space))
		buf.WriteString(`">`)
...
// close completes the multistatus response
func (w *multistatusWriter) close() error {
	...
	if w.responseDescription != "" {
		buf.WriteString("<D:responsedescription>")
		xml.EscapeText(&buf, []byte(w.responseDescription))
		buf.WriteString("</D:responsedescription>")
	}
