...
export default function S3CredentialsManager() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await createS3Credential(form);
      await loadData();
      setForm({ accountId: "", description: "", permissions: ["read", "write"] });
      setShowForm(false);
      toast.success("S3 凭证创建成功");
    } catch (err) {
  ...
  const handleToggleActive = async (cred: S3Credential) => {
    try {
      await updateS3Credential(cred.id, {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("确定要删除此 S3 凭证吗？删除后使用此凭证的应用将无法访问。")) return;
    try {
      await deleteS3Credential(id);
      await loadData();
      toast.success("S3 凭证已删除");
    } catch (err) {
        ...
        {credentials.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p>暂无 S3 凭证</p>
              <p className="text-sm">创建凭证以使用 S3 兼容客户端访问</p>
            </CardContent>
          </Card>
        ) : (
          <>
            {paginatedCredentials.map((cred) => (
              <Card key={cred.id} className={!cred.isActive ? "opacity-60" : ""}>
                <CardContent className="p-4 space-y-3">
                  ...
                  {/* Bucket Name */}
                  <div className="flex items-center gap-2">
                    <Label className="text-xs w-24 shrink-0">Bucket Name:</Label>
                    ...
                    <Button
                      ...
                      size="icon"
                      className="shrink-0"
                      onClick={() => handleCopy(getBucketName(cred.accountId), cred.id + "-bucket")}
                    >
                      {copiedField === cred.id + "-bucket" ? (
                  ...
                  {/* Access Key */}
                  <div className="flex items-center gap-2">
                    <Label className="text-xs w-24 shrink-0">Access Key:</Label>
                    <code className="flex-1 p-2 bg-secondary rounded text-xs font-mono break-all select-all">
                      {cred.accessKeyId}
                    </code>
                    <Button
                      ...
                      size="icon"
                      className="shrink-0"
                      onClick={() => handleCopy(cred.accessKeyId, cred.id + "-ak")}
                    >
                      {copiedField === cred.id + "-ak" ? (
                        <Check className="h-4 w-4" />
                      ) : (
                        <Copy className="h-4 w-4" />
                      )}
                  ...
                  {/* Secret Key */}
                  <div className="flex items-center gap-2">
                    <Label className="text-xs w-24 shrink-0">Secret Key:</Label>
                    <code className="flex-1 p-2 bg-secondary rounded text-xs font-mono break-all select-all">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="shrink-0"
                      onClick={() => handleCopy(cred.secretAccessKey, cred.id + "-sk")}
                    >
                      {copiedField === cred.id + "-sk" ? (
                        <Check className="h-4 w-4" />
                      ) : (
                        <Copy className="h-4 w-4" />
                      )}
